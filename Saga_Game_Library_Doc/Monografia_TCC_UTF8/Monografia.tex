...
\include{capa}    % Capa (não entra na contagem de páginas)
\include{folhaderosto}  % Folha de Rosto
\include{folhadeaprovacao}  % Folha de Aprovação
\include{dedicatoria}   % Dedicatória (opcional)
\include{agradecimentos}  % Agradecimentos (opcional)
\include{epigrafe}   % Epígrafe (opcional)
\include{resumo}    % Resumo em português
\include{abstract}   % Abstract em inglês
\include{sumario}   % Inclui o sumário e as listas de figuras,
...
% Corpo da Monografia
% ----------------------------------------------------------------
\include{Cap_01}                   %% INTRODUCAO
\include{Cap_02}
\include{Cap_03}                   %% Como escrever um projeto de pesquisa
