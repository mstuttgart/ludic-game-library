...
void TMXTileMap::initTilesLayers( const TMXLoader& loader )
{
	...
	// Recebemos uma instancia de VideoManager para usar suas dimensoes
...
bool TMXTileMap::hasLayer ( const String& name )
{
	...
	map<String, TiledLayer*>::iterator it = tiledLayers.find ( name );

	return it != tiledLayers.end() ? true : false;
